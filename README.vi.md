...
- Tổng thời gian chạy GitHub Action miễn phí mỗi tháng là 2000 phút
  - Mỗi lần chạy tool chỉ mất khoảng 1 phút (hoặc nhiều hơn nếu có nhiều sách), nên bạn không phải lo việc vượt giới hạn, trừ khi bạn chép *rất-rất-nhiều* sách trong một tháng
...
- [Tạo tài khoản Dropbox](https://www.dropbox.com/referrals/AAB_nRkqghEm94wvV6A8TNxakMs3Z6qMh3U) (link giới thiệu, nếu tạo tài khoản bằng link này và cài đặt app Dropbox trên pc/mobile thì mỗi tài khoản sẽ được tặng thêm 500MB dung lượng trọn đời, khoảng 100 eBook)

### Thiết bị
Nếu bạn đang dùng Kobo Forma, Sage, Elipsa, Elipsa 2E hoặc Libra Colour thì có thể bỏ qua bước này, vì máy đã có sẵn tính năng Dropbox.
...
## III. Cách chạy tool để convert sách

1. Upload sách chuẩn `epub` vào thư mục `Apps/Rakuten Kobo` của tài khoản Dropbox (không để sách vào thư mục con của thư mục này):
...
2. Để chạy tool thực hiện việc convert sách, trong GitHub, hãy vào `Actions > Convert books`, nhấn nút `Run workflow`, và nhấn tiếp nút màu xanh `Run workflow` một lần nữa ([link trực tiếp](../../actions/workflows/convert.yml))
